...
class mts_related_posts_widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if(is_singular()){
			...
			echo self::get_cat_posts( $qty, $comment_num, $date, $show_thumb6, $box_layout, $show_excerpt, $excerpt_length );
			echo $after_widget;
		}
	}

	public function get_cat_posts( $qty, $comment_num, $date, $show_thumb6, $box_layout, $show_excerpt, $excerpt_length ) {
	...
	foreach($cats as $individual_cat)
		...
		while ( $posts->have_posts() ) { $posts->the_post(); ?>
			<?php echo $open_li_item; ?>
				<?php if ( $show_thumb6 == 1 && has_post_thumbnail() ) : ?>
				<div class="post-img">
